...
No whitespace between tokens is **not** required for signing; Node and Swift both serialize with default spacing rules. **Interoperability rule:** use the same serialization as [`Ombifest/src/canonical.js`](src/canonical.js) (`JSON.stringify({ pins, validUntil, version })` with pins already sorted).
...
- Public key in app: **`OMBIST_PIN_MANIFEST_PUBLIC_KEY_HEX`** — **64 hex chars** = raw 32-byte Ed25519 public key (same extraction as `sign-pin-manifest.mjs` from SPKI).
